...
begin
	...
	ooLoop: for oo as cnv_funcionarios dynamic scroll cursor for
		select 1 as w_i_entidades,cdMatricula as w_cdMatricula, SqContrato as w_SqContrato,CdPessoa as w_CdPessoa,date(DtAdmissao) as w_dt_admissao,cdAdmissao as w_cdAdmissao,
			   ...
			   upper(inRais) as w_sai_rais,tecbth_delivery.fu_tirachars(tecbth_delivery.fu_tirachars(nrContaCorrente,'X'),'.') as w_nrContaCorrente,dgContaCorrente as w_dgContaCorrente,NrBancoContaCorrente as w_i_bancos,NrAgenciaContaCorrente as w_i_agencias,
		...
		from tecbth_delivery.gp001_funcionario where w_cdmatricula  in (1481)
		order by 1,2,4 asc		
...
provimento = ( case  when CdTpProvimento = 85100 then 1 
 ...
 from tecbth_delivery.gp001_CARGO  where cdcargo = w_cargos;
